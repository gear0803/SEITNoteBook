...
dt=dx/(4.0*cc);     %时间差分步长，此处为走过1/4个空间差分步长的时间（走过2.5m的时间），可修改，但不得大于CFL条件
                    %一个dt为走过1/40波长的时间，所以40dt为走过一个波长的时间
nmax=200;           %总时间迭代次数5T = 5*40dt = 200dt
...
xlabel('Time step');
ylabel('Amplitude of Ex');
title('dt=d/(4*c)  z = \lambda')

figure;
plot(wavnum2)
xlabel('Time step');
ylabel('Amplitude of Ex');
title('dt=d/(4*c)  z = 2\lambda')

%% 解释 时间分辨率为：dt=d/(4*c)  
%{
Z=0：
波谷的周期为40个timestep，一个dt为走过1/40波长的时间，所以40dt为走过一个波长的时间
波的幅值为1，因为实在发射源处

Z=\lambda 一个波长:
1、在波长为1时，理论要40个timestep才能有信号，因为要经过一个波长的时间才能传到
...
3、在40~80个timestep时波形不稳定，这是因为发射源不是由电流源激励，
    所以波形会有大的跳变，跳完就稳定了 所以后面周期差为40timestep
4、幅值相比于Z=0处变小，是因为信号在空间传输的损耗引起的，满足物理规律

Z = 2*\lambda 两个波长：
分析与Z=1个波长类似：
1、在波长为2时，理论要80个timestep才能有信号，因为要经过2个波长的时间才能传到
2、但其实21步就有数据，是因为时间步长为1到20时源信号还没有传过去 
    而21timestep时存在很小幅值的信号，该超光速存在是因为计算机在数值计算中存在数值离散，
    中心差分导致的，而可以忽略不计是因为算法本身设置就可以大致满足物理规律，
    因此可以当做光速是可以传到80才开始有信号
3、在80~120个timestep时波形不稳定，这是因为发射源不是由电流源激励，
    所以波形会有大的跳变，跳完就稳定了 所以后面周期差为40timestep
